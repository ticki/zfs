...
// zio_compress os_compress;
// uint8_t os_copies;
// zio_checksum os_dedup_checksum;
...
//
// os->os_compress = zio_compress_select(os->os_spa, newval, ZIO_COMPRESS_ON);
// }
//
// static void
...
//
// Inheritance and range checking should have been done by now.
// ASSERT(newval == ZFS_CACHE_ALL || newval == ZFS_CACHE_NONE || newval == ZFS_CACHE_METADATA);
//
...
//
// Inheritance and range checking should have been done by now.
// assert!(newval == ZFS_SYNC_STANDARD || newval == ZFS_SYNC_ALWAYS || newval == ZFS_SYNC_DISABLED);
//
// os->os_sync = newval;
// if (os->os_zil)
...
//
// int
// dmu_objset_open_impl(spa_t *spa, dsl_dataset_t *ds, blkptr_t *bp, objset_t **osp)
// {
// objset_t *os;
// int i, err;
//
...
// err = arc_read(NULL, spa, os->os_rootbp, arc_getbuf_func, &os->os_phys_buf,
...
// }
//
// fn dmu_dir_list_next(objset_t *os, int namelen, char *name, uint64_t *idp, uint64_t *offp) -> zfs::Result<()> {
